...
test.describe('Playwright site - Search feature (firstTest)', () => {
  ...
  test('検索ボタンの表示確認', async ({ page }) => {
    const searchButton = page.getByRole('button', { name: /Search|検索|Open search/i }).first();
    ...
    const fallback = page.locator('[aria-label*="search"], [placeholder*="Search"], [data-search]').first();
  ...
  test('検索を実行して結果を確認 (Installation)', async ({ page }) => {
    ...
    const resultLink = page.getByRole('link', { name: /Installation|Install/i }).first();
