...
## 概要
目的：公式サイト `https://playwright.dev/` の検索機能（ヘッダー/ページ内の検索UI、検索モーダル、ショートカット、検索結果）を検証する。テストは Playwright を使って自動化可能な手順で記載する。シードファイルとして `tests/seed.spec.ts`（または `tests/seed.spec.js`）を利用して、以下の項目をカバーする。
...
test('Search UI: open search and find Installation', async ({ page }) => {
  ...
  const searchButton = page.getByRole('button', { name: /Search|検索/i }).first();
  ...
  const searchInput = page.locator('input[type="search"], input[role="searchbox"], [placeholder*="Search"]');
  ...
  // wait for results
  const result = page.getByRole('link', { name: /Installation|Install/i }).first();
